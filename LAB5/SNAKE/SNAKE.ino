...
void loop() {
  ...
  for (byte i = snake_length - 1; i >= 1; i--) {
    snake[i] = snake[i-1];
  }
  snake[0] = scaled_value_x * 8 + scaled_value_y;
...
void FillLEDs_in_matrix() {
  ...
  for (byte i = 0; i < snake_length; i++) {
    leds[snake[i]] = ColorFromPalette(myRedWhiteBluePalette_p, 1, 255, currentBlending);
  }
}
