...
UltraSonicDistanceSensor jobboldali_szenzor(19,18); //trigger white in 19 and echo 18 in green
UltraSonicDistanceSensor baloldali_szenzor(17,16);  //trigger white in 17 and echo 16 in green
float jobboldali_tav, baloldali_tav;

//line following
QTRSensors qtr;
const uint8_t SensorCount = 8;
uint16_t sensorValues[SensorCount];
byte sensorValue_lebutitott[SensorCount];
...
{ 
  ...
  jobboldali_tav = jobboldali_szenzor.measureDistanceCm();
  baloldali_tav = baloldali_szenzor.measureDistanceCm();

  //Serial.print(" left = ");
 // Serial.println(baloldali_tav);
  lcd.setCursor(0,0);
  //lcd.print("bal = ");
  if (baloldali_tav<100)
    {
    lcd.print(baloldali_tav,2);
    lcd.print("   ");
    }
  else lcd.print("FREE");
  delay(25);  
  ...
  lcd.setCursor(10,0);
  //lcd.print("jobb = ");
  if (jobboldali_tav<100)
    {
    lcd.print(jobboldali_tav,2);
    lcd.print("   ");
    }
  else lcd.print("FREE");  
  delay(25);  
...
*/
    ...
    sensorValue_lebutitott[i] = sensorValues[i] /128;
    
    Serial.print(sensorValue_lebutitott[i]);
    Serial.print('\t');
  }
  Serial.println();
  ...
  //show the IR results
   ...
   lcd.write(sensorValue_lebutitott[i]);
